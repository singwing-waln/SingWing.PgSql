...
/// <summary>
/// Represents a parameter whose value is a nullable array type value.
/// </summary>
...
internal sealed class NullableArrayParameter<TElement, TProtocol> : Parameter
    ...
    where TProtocol : IDataTypeProtocol<TElement, TProtocol>
{
    ...
    /// </summary>
    private static readonly NullableArrayParameter<TElement, TProtocol> Null = new(null);

    /// <summary>
    /// The parameter whose value is an empty array.
    /// </summary>
    private static readonly NullableArrayParameter<TElement, TProtocol> Empty = new(Array.Empty<TElement?>());
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="NullableArrayParameter{TElement, TProtocol}"/> class 
    /// with a collection of TElement? values.
    /// </summary>
    /// <param name="elements">The collection of TElement? values.</param>
    internal NullableArrayParameter(IEnumerable<TElement?>? elements)
    {
    ...
    public override object? Value
    {
        ...
        set
        {
            ...
            {
                _binaryLength = GetArrayBinaryLengthForSending(nullableElements);
                _elements = nullableElements;
                return;
            }

            if (value is IEnumerable<TElement> enumerable)
            {
                var elements = ToNullable(enumerable);
                _binaryLength = GetArrayBinaryLengthForSending(elements);
                _elements = elements;
                return;
            }
            ...
            static IEnumerable<TElement?> ToNullable(IEnumerable<TElement> enumerable)
            {
            ...
            static IEnumerable<TElement?> ObjectsToElements(IEnumerable enumerable)
            {
                ...
                foreach (var item in enumerable)
                {
                    yield return Conversion.ToElement(item, dataType) as TElement?;
                }
            }
        }
    }
    ...
    public override Parameter Clone() =>
        ...
        ? this
        : (Parameter)new NullableArrayParameter<TElement, TProtocol>(_elements);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (TProtocol.IsElementBinaryLengthFixed)
        {
            ...
            foreach (var element in _elements)
            {
                if (element is null)
                {
                    await writer.WriteInt32Async(-1, cancellationToken);
                }
                else
                {
                    await writer.WriteInt32Async(elementBinaryLength, cancellationToken);
                    await TProtocol.WriteElementAsync(element.Value, writer, cancellationToken);
                }
            }
        }
        else
        {
            foreach (var element in _elements)
            {
                ...
                {
                    await writer.WriteInt32Async(TProtocol.ElementBinaryLengthOf(element.Value), cancellationToken);
                    await TProtocol.WriteElementAsync(element.Value, writer, cancellationToken);
                }
            }
        }
    ...
    /// <summary>
    /// Creates or returns an instance of the <see cref="NullableArrayParameter{TElement, TProtocol}"/> class 
    ...
    /// a new instance of the <see cref="NullableArrayParameter{TElement, TProtocol}"/> class otherwise.
    /// </returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static NullableArrayParameter<TElement, TProtocol> From(IEnumerable<TElement?>? elements) =>
        elements is null
        ? Null
        : elements.IsEmpty()
        ? Empty :
        new NullableArrayParameter<TElement, TProtocol>(elements);
    ...
    private static int GetArrayBinaryLengthForSending(in IEnumerable<TElement?>? elements)
    {
        ...
        if (TProtocol.IsElementBinaryLengthFixed)
        {
            ...
            {
                length += sizeof(int) + (element is null ? 0 : elementBinaryLength);
            }
        }
        else
        {
            foreach (var element in elements)
            {
                length += sizeof(int) + (element is null ? 0 : TProtocol.ElementBinaryLengthOf(element.Value));
            }
        }
